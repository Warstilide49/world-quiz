...
const Play = ({ allChoices }) => {
  ...
  useEffect(() => {
    ...
    const countries = allChoices[LEVELS.findIndex((element) => element === level)] || [];
  ...
  return (
    <div className="flex flex-column" id="play">
      ...
      { !showUserModal && Object.keys(flagsToFind).map((code) => (
            <div className="flag-container" key={code}>
              ...
              />
              {flagsToFind[code].isItRight && flagsToFind[code].hasBeenSelected && <img src={tickMark} className="flag-result" alt="Correct" width="20px" height="20px"></img>}
              {!flagsToFind[code].isItRight && flagsToFind[code].hasBeenSelected && <img src={wrongMark} className="flag-result" alt="Wrong" width="20px" height="20px"></img>}
            </div>
          ))}
        
      </div>
      <Map width="85vw" height="85vh" flagsToFind={flagsToFind} setFlagsToFind={setFlagsToFind} showUserModal={showUserModal} />
    </div>
  );
};
